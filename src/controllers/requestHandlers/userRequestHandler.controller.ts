...
const postUserRequestHandler: RequestHandler<
    {},
    ResponseUserAttribute | { errorMessage: string },
    RequestUserAttribute
> = async (req, res) => {
    try {
        const inputRequestUser = req.body;

        const { data: createdUser, statusCode } = await UserService.createUser(
            inputRequestUser
        );
        ...
        const errorMessage = ErrorHandler.handleUnknownError(error);
        return res.status(500).send({ errorMessage });
    }
};

const getAllUsersRequestHandler: RequestHandler<
    {},
    { users: ResponseUserAttribute[] } | { errorMessage: string }
> = async (req, res) => {
    try {
        const { data: fullUsers, statusCode } = await UserService.getAllUsers();

        const responseUsers: ResponseUserAttribute[] = fullUsers.map(
            (fullUser) => {
...
const getUserRequestHandler: RequestHandler<
    { user_id: string },
    ResponseUserAttribute | { errorMessage: string }
> = async (req, res) => {
    try {
        ...
        const {
            ...
            statusCode,
            errorMessage,
        } = await UserService.getUser(inputUsername);
...
const updateUserRequestHandler: RequestHandler<
    { user_id: string },
    ResponseUserAttribute | { errorMessage: string },
    RequestUserAttribute
> = async (req, res) => {
    try {
        ...
        const {
            ...
            statusCode,
            errorMessage,
        } = await UserService.updateUser(inputFullUser);
...
const deleteUserRequestHandler: RequestHandler<
    { user_id: string },
    { errorMessage: string }
> = async (req, res) => {
    try {
        ...
        const { statusCode, errorMessage } = await UserService.deleteUser(
            userId
        );
