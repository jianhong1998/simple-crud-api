        ...
        return (req, res, next) => {
            try {
                ...
                if (typeof authorization === 'undefined') {
                    // 401 - Unauthorized
                    return res.status(401).send(new ErrorResponse('Authorization is undefined in request headers.'));
                }
    
                if (authorization.split(' ').length < 2) {
                    return res.status(401).send(new ErrorResponse('Authorization does not contain token.'));
                }
