...
export default class EmployeeRequestVerificationService {
    public static verifyDepartment(department: unknown): boolean {
        return typeof department === "string" && Object.values(Department).includes(department.toUpperCase() as Department);
    }

    public static verifyEmployeeId(employeeId: unknown): boolean {
        return typeof employeeId === "string" && NumberVerifier.isStringValidNumber(employeeId) && parseInt(employeeId) > 0;
    }
