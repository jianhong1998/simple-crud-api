...
export default class LoginService {
    public static async login(inputUser: LoginUserAttribute): Promise<DataResponse<LoginResult>> {
        return new Promise(async (resolve, reject) => {
            try {
                const { username: inputUsername, password: inputPassword } = inputUser;
                ...
                if (existingUserDataModel === null) {
                    return resolve(new DataResponse({token: ''}, 404, 'User Not Found.'));
                }
        
                const { password: existingPassword, username, departmentId, userId } = existingUserDataModel;

                const passwordCompaereResult = await PasswordService.comparePassword(inputPassword, existingPassword);

                // 401 - Unauthorised
                if (!passwordCompaereResult) {
                    return resolve(new DataResponse({token: ''}, 401, 'Password incorrect.'));
                }

                const token = TokenService.generateToken({userId, username, departmentId} as ResponseUserAttribute);
